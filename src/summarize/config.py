...
def _load_json(path: Path) -> Mapping[str, Any]:
    with path.open("r", encoding="utf-8") as f:
        try:
            ...
            raise ValueError(f"Failed to parse JSON configuration at {path}: {exc}") from exc

    if not isinstance(data, Mapping):
        raise ValueError("Top-level JSON configuration must be an object")

    return data


def load_config(path: str | os.PathLike[str] | None = None) -> AppConfig:
    """Load application configuration from JSON or environment variables."""

    config_data: Mapping[str, Any]
    if path is not None:
        config_data = _load_json(_coerce_path(path))
    else:
        default_path = Path.cwd() / "config.json"
        config_data = _load_json(default_path) if default_path.exists() else {}
